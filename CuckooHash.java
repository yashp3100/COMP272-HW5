...
public class CuckooHash<K, V> {
    ...
    public void put(K key, V value) {
        int count = 0; // get the number of times the element is moved
        K CurKey = key;
        V CurValue = value;

        while (count < CAPACITY) {
            //put value in hash1 position
            int one = hash1(CurKey);

            if (table[one] == null) {
                //position is empty then enter the key value
                table[one] = new Bucket<>(CurKey, CurValue);
                return;
            } else if (table[one].getBucKey().equals(CurKey) && table[one].getValue().equals(CurValue)) {
                //Duplicate the key with the same value
                return;
            }
            //Get rid of the value at the first position
            K replaceKey = table[one].getBucKey();
            V replaceValue = table[one].getValue();
            table[one] = new Bucket<>(CurKey, CurValue); // Inserts new value

            //updates the current key with the replaced key
            CurKey = replaceKey;
            CurValue = replaceValue;

            //Place the replaced pair in the position given by hash2
            int two = hash2(CurKey);

            if (table[two] == null) {
                //if the position is empty then replace with the kicked pair
                table[two] = new Bucket<>(CurKey, CurValue);
                return;
            } else if (table[two].getBucKey().equals(CurKey) && table[two].getValue().equals(CurValue)) {
                //Duplicate key with the same value
                return;
            }

                //update the key pair to position two
                replaceKey = table[two].getBucKey();
                replaceValue = table[two].getValue();
                table[two] = new Bucket<>(CurKey, CurValue);
